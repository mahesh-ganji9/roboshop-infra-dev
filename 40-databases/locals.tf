locals {
  ...
  mysql_role_name = join("-", [for name in ["${var.var.project}","${var.env}",mysql-role]: title(name)])
  mysql_role_policy = join("-", [for name in ["${var.var.project}","${var.env}",mysql-policy]: title(name)])
